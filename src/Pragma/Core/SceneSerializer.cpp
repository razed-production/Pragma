...
{
SerializedScene LoadSceneFromFile(const std::filesystem::path& path)
{
    ...
    Pragma::Core::Log(Pragma::Core::LogCategory::Scene, Pragma::Core::LogLevel::Info, "Loading scene file: " + path.string());
    ...
    auto parseError = [&](const std::string& message) -> std::runtime_error
    {
        return std::runtime_error(
            "Scene parse error in '" + path.string() + "' at line " + std::to_string(lineNumber) + ": " + message);
    };

    auto commitCurrentBlock = [&]()
    {
        if (currentBlock == ParseBlock::SceneObject)
        {
            if (currentObject.Id == Pragma::Renderer::InvalidEntityId)
            {
                throw parseError("scene_object is missing an id.");
            }
            if (currentObject.Name.empty())
            {
                throw parseError("scene_object is missing a name.");
            }
            if (currentObject.ParentId == currentObject.Id)
            {
                throw parseError("scene_object cannot parent itself.");
            }
            if (currentObject.IsActiveCamera && !currentObject.Camera.has_value())
            {
                throw parseError("scene_object marks itself as active camera but has no camera component.");
            }
            if (currentObject.CameraController.has_value() && !currentObject.Camera.has_value())
            {
                throw parseError("scene_object contains CameraController without Camera.");
            }
            if (currentObject.ManagedScriptProjectAsset.empty() != currentObject.ManagedScriptTypeName.empty())
            {
                throw parseError("scene_object managed script requires both project and type.");
            }
            if (!currentObject.ScriptName.empty() && !currentObject.ManagedScriptProjectAsset.empty())
            {
                throw parseError("scene_object cannot define native and managed script at the same time.");
            }
            if (currentObject.MeshRenderer.has_value() && currentObject.MeshRenderer->MeshAsset.empty())
            {
                throw parseError("scene_object contains MeshRenderer without mesh asset.");
            }
            if (currentObject.MeshRenderer.has_value() && currentObject.MeshRenderer->MaterialAsset.empty())
            {
                throw parseError("scene_object contains MeshRenderer without material asset.");
            }
    ...
    while (std::getline(input, line))
    {
        ...
        if (line == "scene_object")
        {
            if (!versionDefined)
            {
                throw parseError("version must be declared before scene_object blocks.");
            }
            commitCurrentBlock();
            currentBlock = ParseBlock::SceneObject;
            continue;
        }
        ...
        if (currentBlock == ParseBlock::None)
        {
            if (key == "version")
            {
                ...
                if (version < kMinimumSupportedSceneVersion || version > kLatestSceneVersion)
                {
                    throw parseError("unsupported scene version " + std::to_string(version) + '.');
                }
        ...
        else if (currentBlock == ParseBlock::SceneObject)
        {
            ...
            else if (key == "prefab.asset")
            {
                currentObject.PrefabAssetId.Value = value;
            }
            ...
            else if (key == "camera.active")
            {
                currentObject.IsActiveCamera = ParseBool(value);
            }
            ...
            else if (key == "managed_script.project")
            {
                currentObject.ManagedScriptProjectAsset.Value = value;
            }
            else if (key == "managed_script.type")
            {
                currentObject.ManagedScriptTypeName = value;
            }
            ...
            {
                throw parseError("unknown scene_object key '" + key + "'.");
            }
        }
    }
    ...
    if (!versionDefined)
    {
        throw std::runtime_error("Scene parse error in '" + path.string() + "': missing required version.");
    }

    if (scene.Objects.empty())
    {
        throw std::runtime_error("Scene file does not contain any objects.");
    }

    std::size_t activeCameraCount = 0;
    std::size_t lightCount = 0;
    for (const SerializedSceneObject& object : scene.Objects)
    {
        if (object.IsActiveCamera)
        {
            ++activeCameraCount;
        }
        ...
        if (object.RigidBody.has_value() != object.BoxCollider.has_value())
        {
            throw std::runtime_error(
                "Scene parse error in '" + path.string() + "': rigid body and box collider must be added together for this build.");
        }
    }

    if (activeCameraCount != 1)
    {
        throw std::runtime_error("Scene parse error in '" + path.string() + "': expected exactly one active camera.");
    }

    if (lightCount > 1)
    {
        throw std::runtime_error("Scene parse error in '" + path.string() + "': expected at most one directional light component.");
    }
    ...
    for (const SerializedSceneObject& object : scene.Objects)
    {
        ...
        for (const SerializedSceneObject& candidate : scene.Objects)
        {
        ...
        {
            throw std::runtime_error(
                "Scene parse error in '" + path.string() + "': object '" + object.Name + "' references a missing parent.");
        }
    }
    ...
    for (const SerializedSceneObject& object : scene.Objects)
    {
        ...
        while (currentParent != Pragma::Renderer::InvalidEntityId)
        {
            ...
            {
                throw std::runtime_error(
                    "Scene parse error in '" + path.string() + "': hierarchy cycle detected.");
            }
...
void SaveSceneToFile(const SerializedScene& scene, const std::filesystem::path& path)
{
    ...
    output << "# Pragma scene file\n";
    output << "version=" << kLatestSceneVersion << "\n\n";
    for (const SerializedSceneObject& object : scene.Objects)
    {
        output << "scene_object\n";
        output << "id=" << object.Id << '\n';
        ...
        {
            output << "prefab.asset=" << object.PrefabAssetId.Value << '\n';
        }
        ...
        if (object.Camera.has_value())
        {
            ...
            output << "camera.active=" << FormatBool(object.IsActiveCamera) << '\n';
        }
        ...
        if (!object.ManagedScriptProjectAsset.empty())
        {
            output << "managed_script.project=" << object.ManagedScriptProjectAsset.Value << '\n';
            output << "managed_script.type=" << object.ManagedScriptTypeName << '\n';
        }
